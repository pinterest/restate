...
fn bench_pipelined(c: &mut Criterion) {
    ...
    // Each sample runs on a fresh runtime with a fixed worker count, built
    // outside the timed region. Where the OS places the worker threads (e.g.
    // P- vs E-cores) dominates cross-thread wakeup cost, so a placement
    // decided once per process would bias the whole run; re-rolling it per
    // sample averages the placement lottery into every run instead.
    let fresh_rt = || {
    ...
    // Iterations are too long (~15ms) for linear sampling to fit its schedule.
    group.sampling_mode(SamplingMode::Flat);
    // Measured run-to-run drift is ~5% even on a quiet, pinned machine; don't
    // report smaller swings as regressions.
