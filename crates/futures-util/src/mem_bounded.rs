...
impl<T> Receiver<T> {
    ...
    /// - If a byte limit is set and the total size of the received elements exceeds the limit.
    ///   NOTE: The returned batch can overshoot the limit by at most one element.
    /// - If a max count is set and the number of received elements reaches the limit.
    /// - If neither a size or byte limit are set, this will drain all the elements that are available in the channel at the moment
    ...
    ///
    /// Returns a tupe of the number of elements added to the buffer and the number of bytes received.
    ///
    ...
    pub fn try_recv_many_with_buf(
        ...
        mut max_count: Option<usize>,
    ) -> (usize, usize) {
        if byte_limit.is_none() && max_count.is_none() {
            max_count = Some(self.rx.len());
        }
        if byte_limit.is_none()
            && let Some(max_count) = max_count
        {
            // If we have a max count set, and no byte limit, it's fair to
            // just reserve the vector capacity capacity beforehand.
            buf.reserve(max_count.min(self.rx.len()));
        }
        let max_count = max_count.unwrap_or(usize::MAX);
        let max_bytes = byte_limit.unwrap_or(usize::MAX);

        let mut total_bytes = 0;
        let mut total_count = 0;
        while total_count < max_count && total_bytes < max_bytes {
        ...
        self.semaphore.add_permits(total_bytes);
...
mod tests {
    ...
    async fn try_recv_many_with_buf() {
        ...
        // Count only limit
        {
            let (_tx, mut rx) = prep_full_channel().await;
            let mut buf = Vec::new();
            let (count, bytes) = rx.try_recv_many_with_buf(&mut buf, None, Some(3));
            assert_eq!(count, 3);
            assert_eq!(bytes, 15);
        ...
        // Byte only limit
        {
            ...
            let mut buf = Vec::new();
            let (count, bytes) =
                rx.try_recv_many_with_buf(&mut buf, /* 13 bytes */ Some(7), None);
            assert_eq!(count, 2);
            assert_eq!(bytes, 10);
        ...
        // Byte and count limit
        {
            ...
            let mut buf = Vec::new();
            let (count, bytes) =
                rx.try_recv_many_with_buf(&mut buf, /* 13 bytes */ Some(13), Some(4));
            assert_eq!(count, 3);
            assert_eq!(bytes, 15);
            assert_eq!(buf, vec![Sized(5); 3]);
        }
        ...
        // No limit (drain)
        {
            let (_tx, mut rx) = prep_full_channel().await;
            let mut buf = Vec::new();
            let (count, bytes) = rx.try_recv_many_with_buf(&mut buf, None, None);
            assert_eq!(count, 10);
            assert_eq!(bytes, 50);
