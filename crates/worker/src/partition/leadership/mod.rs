    ...
    T: TransportConnect,
{
    ...
    pub(super) fn should_process_rpc(&self) -> bool {
        // In case of being candidate or becoming a leader we prefer to park RPC requests
        // until we transition out of the current state.
        matches!(self.state, State::Leader(_) | State::Follower)
    }
    ...
    pub async fn maybe_step_down(
        ...
        let planned_mode = match &self.state {
            ...
            State::Candidate { leader_epoch, .. } | State::BecomingLeader { leader_epoch, .. } => {
